...
import { createCashToCardConversion } from "@/src/features/transactions/transaction-service";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    await createCashToCardConversion(businessId, date, notes, amount);

    return new Response(
      JSON.stringify({ message: "Cash to card conversion added" }),
      { status: 201, headers: { "Content-Type": "application/json" } },
    );
  } catch (error) {
    console.error("Failed to add cash-to-card conversion:", error);
